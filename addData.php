...
for ($i = 0; $i < 10; $i++){
    ...
    $query = "INSERT INTO users (`name`, `last_name`, `email`, `password`, `ip`, `token`, `avatar`, `banner`, `schools_id`) VALUES ('{$faker->firstname}','{$faker->lastname}','{$faker->email}','$test','{$faker->ipv4}','{$faker->ipv4}','{$faker->imageUrl($width = 640, $height = 480)}','{$faker->imageUrl($width = 640, $height = 480)}','{$faker->numberBetween(1,10)}')";
    $statement = $pdo->exec($query);
};
