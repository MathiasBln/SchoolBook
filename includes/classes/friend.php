...
class Friend{
    ...
    public function is_already_friends($my_id, $user_id){
        try{
            $sql = "SELECT * FROM `relation` WHERE (user_one = :my_id AND user_two = :frnd_id) OR (user_one = :frnd_id AND user_two = :my_id)";
    ...
    public function make_friends($my_id, $user_id){
        ...
        try{
            ...
            if($delete_stmt->execute()){
                ...
                $sql = "INSERT INTO `relation`(user_one, user_two) VALUES(?, ?)";
                $stmt = $this->db->prepare($sql);
                $stmt->execute([$my_id, $user_id]);
                header('Location: user_profile.php?id='.$user_id);
                exit;
    ...
    public function delete_friends($my_id, $user_id){
        try{
            $delete_friends = "DELETE FROM `relation` WHERE (user_one = :my_id AND user_two = :frnd_id) OR (user_one = :frnd_id AND user_two = :my_id)";
    ...
    public function request_notification($my_id, $send_data){
        try{
            $sql = "SELECT sender, username, avatar FROM `friend_request` JOIN users ON friend_request.sender = users.iduser WHERE receiver = ?";
    ...
    public function get_all_friends($my_id, $send_data){
        try{
            $sql = "SELECT * FROM `relation` WHERE user_one = :my_id OR user_two = :my_id";
            $stmt = $this->db->prepare($sql);
            $stmt->bindValue(':my_id',$my_id, PDO::PARAM_INT);
            $stmt->execute();
                ...
                if($send_data){
                    ...
                    foreach($all_users as $row){
                        if($row->user_one == $my_id){
                            $get_user = "SELECT iduser, username, avatar FROM `users` WHERE iduser = ?";
